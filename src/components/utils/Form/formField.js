...
import CheckboxGroup from '../Form/inputs/checkboxGroup';
import RadioGroup from '../Form/inputs/radiogroup';
import SelectWithFilter from '../Form/inputs/selectWithFilter';
            ...
            case ('selectwithfilter'):
                formTemplate = (
                    ...
                    <SelectWithFilter
                        items = {formdata.config.options}
                        onChange = {(event) => change({ event, id })}
                        placeholder = {formdata.value || formdata.config.placeholder} //show selected value or placeholder text
                        // selected = {formdata.value}
                    />
                    {formdata.validationMessage && formdata.config.showBottomValidation ? <div className="bottom_validation_message">{showError()}</div> :null}
                    </div>
